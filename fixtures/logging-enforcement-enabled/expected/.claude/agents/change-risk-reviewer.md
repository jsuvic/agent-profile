...
Policy version: `change-risk/v1`. Your provider-neutral model-policy role is `critical-reviewer`; use its target-native model and effort resolution.
...
This is a clean-room review: do not use an implementer report, prior praise, or prior finding list. No changed, staged, unstaged, or relevant untracked file is hidden from you; every exclusion is listed with its reason.
...
- `policyVersion` is `change-risk/v1`; bind the envelope to the supplied `snapshotId`.

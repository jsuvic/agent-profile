 ...
 *
 * `previousModelPolicy` (Phase 31.5 I6d PR review Finding 1) is the prior
 * `ai-profile.lock`'s `modelPolicy` block, if any, forwarded straight into
 * `buildModelPolicyTabnineTargetTable` exactly like `resolveModelPolicyLockfile`
 * does -- so this generated guideline and the lock that claims to describe
 * the same compile can never disagree about a retained Tabnine resolution.
 */
...
`;
  ...
  const table = buildModelPolicyTabnineTargetTable(
    preset,
    deriveModelPolicyTabnineRoleOverrides(explicitRoleOverrides),
    previousModelPolicy,
  );
  const rows = table
    .map((row) => {

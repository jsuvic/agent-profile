...
function createChangeRiskReviewer(): AiProfileSubagent {
  ...
  // Every value the validator requires to be a record, rendered from the
  // projection so a newly *required* structured field cannot reach the prompt
  // unstated. A newly optional record-valued field is outside that guard; see
  // the derivation test in compiler.test.ts.
  const structuredShapes = result.structuredFieldShapes
    .map((shape) => {
      const optional =
        shape.optionalKeys.length > 0
          ? ` (${quotedList(shape.optionalKeys, ", ")} optional)`
          : "";
      return `  - \`${shape.field}\`: ${quotedList(shape.keys, ", ")}${optional}.`;
    })
    .join("\n");

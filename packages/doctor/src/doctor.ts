...
const GENERATED_HEADER_PATTERNS = [
  "Generated by Agent Profile Compiler",
];

const TABNINE_UNSAFE_TOOLS = [
  "run_shell_command",
  "write_file",
  "browser",
  "fetch",
...
async function checkSubagentArtifacts(
  ...
  for (const root of SUBAGENT_ROOTS) {
    ...
    for (const relativePath of files) {
      ...
      if (!file.ok) continue;
      const text = decodeUtf8(file.bytes);
      const isGenerated = isGeneratedSubagent(text);
      ...
      if (
        root === ".codex/agents" &&
        /sandbox_mode\s*=\s*"danger-full-access"/u.test(text)
      ) {
        issues.push(
          issue(
            "LINT-SUBAGENT-003",
      ...
      if (
        root === ".claude/agents" &&
        /permissionMode:\s*bypassPermissions/u.test(text)
      ) {
        issues.push(
          issue(
            "LINT-SUBAGENT-004",
      ...
      if (root === ".tabnine/agent/agents") {
        const loweredText = text.toLowerCase();
        for (const tool of TABNINE_UNSAFE_TOOLS) {
          if (loweredText.includes(tool)) {
            issues.push(
              issue(
                "LINT-SUBAGENT-007",
                "warning",
                ...
                `references ${tool}`,
                `${relativePath} references write/shell/browser/network-capable tool ${tool} while Tabnine subagents are experimental/no-confirmation.`,
      ...
      if (
        ...
        issues.push(
          issue(
            ...
            "warning",
            relativePath,
            "claimed by current compile output",
            "orphan generated subagent",
            `${relativePath} carries the generated-file header but is not produced by the current profile.`,
  ...
  await checkSubagentBroadening(rootDir, generatedFiles, issues);

  if (
    profile.clients.tabnine.enabled &&
    enabled.some((agent) => agent.toolScope === "read-only")
  ) {
    issues.push(
      issue(
        "LINT-SUBAGENT-008",
...
async function checkSubagentBroadening(
  rootDir: string,
  generatedFiles: GeneratedFile[],
  issues: DoctorIssue[],
): Promise<void> {
  for (const file of generatedFiles) {
    ...
    if (
      file.target === "claude-subagents" &&
      /tools:.*Bash/u.test(text) === false
    ) {
      // example placeholder: no broadening when no Bash tool present
    }

    if (
      file.target === "codex-subagents" &&
      /sandbox_mode\s*=\s*"workspace-write"/u.test(text)
    ) {
      // workspace-write is allowed only when effective permissions permit; the compiler
      // refuses to emit otherwise. Doctor reports broadening as LINT-SUBAGENT-001 only
      // when the on-disk file diverges from generated bytes — handled by lockfile drift.
    }
  }
}

function isGeneratedSubagent(text: string): boolean {
  return GENERATED_HEADER_PATTERNS.some((pattern) => text.includes(pattern));
}

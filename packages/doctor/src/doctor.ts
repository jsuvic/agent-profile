...
async function checkSubagentArtifacts(input: SubagentCheckInput): Promise<void> {
...
function checkTabnineSubagentFile(
  ...
  const unsafe = tools.filter((tool) =>
    TABNINE_UNSAFE_TOOL_NAMES.has(tool),
  );
  ...
  if (
    tools.includes("run_shell_command") &&
    effective.shell.run !== "allow"
  ) {
    issues.push(
      issue(

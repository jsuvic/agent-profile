...
`;
  ...
  const bypassRoot = await createGeneratedProject({ extraYaml: subagentExtras });
  await writeProjectFile(
    bypassRoot,
  ...
  assertHasIssue(
    await runDoctor({ rootDir: bypassRoot }),
    "LINT-SUBAGENT-004",
  );

  // Codex danger-full-access -> LINT-SUBAGENT-003
  const dangerRoot = await createGeneratedProject({ extraYaml: subagentExtras });
  await writeProjectFile(
    dangerRoot,
  ...
  assertHasIssue(
    await runDoctor({ rootDir: dangerRoot }),
    "LINT-SUBAGENT-003",
  );
  ...
  const tabnineRoot = await createGeneratedProject({ extraYaml: subagentExtras });
  await writeProjectFile(
    tabnineRoot,
  ...
  const orphanRoot = await createGeneratedProject({ extraYaml: subagentExtras });
  await writeProjectFile(
    orphanRoot,
  ...
  const secretRoot = await createGeneratedProject({ extraYaml: subagentExtras });
  await writeProjectFile(
    secretRoot,
    ".claude/agents/code-reviewer.md",

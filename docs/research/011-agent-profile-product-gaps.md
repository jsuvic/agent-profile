...
| Artifact and doctor UI views         | specified but not implemented           | Approved read-only viewer specs exist but current route inventory does not expose them.                      | Phase 6 artifact and doctor viewer specs                                               |
...
The dashboard, profile editor, and migration flow make governance more approachable. The most valuable next UI work is evidence, not broader mutation:

- generated-artifact browser grouped by client and capability;
- doctor findings grouped by severity and ownership;
...
6. Local UI evidence views are incomplete.
...
| O10 | Evidence UI                       | governance requires opening many files                     | ADOPT       | finish artifact and doctor viewers before adding more writes   | P1            | Low    | M      | High                |
...
- Local evidence UI and public capability parity.
...
The next product direction should make the existing lifecycle comprehensible, add repo-local continuity and safe exit/recovery, finish evidence-oriented UI, and then extend integrations through config-only target adapters. These investments strengthen Agent Profile's identity rather than broadening it into a credential broker, environment installer, or generic model router.

...
Verified against official product documentation on 2026-07-02, with the
runtime-permissions row reverified on 2026-07-16 for Phase 31 I8. This matrix
is evidence for target specs; it is not permission to generate a capability
by itself.
...
| Runtime permissions and safety modes | beta permission profiles plus legacy sandbox compatibility | permission modes, merged settings, and sandbox controls             | Auto-approve, Ask first, Disable per native/MCP tool                                                        | [Codex permissions](https://learn.chatgpt.com/docs/permissions); [Claude permissions](https://code.claude.com/docs/en/permissions); [Claude settings](https://code.claude.com/docs/en/settings); [Tabnine agent settings](https://docs.tabnine.com/main/getting-started/tabnine-agent/agent-settings)                                | 2026-07-16 | all `confirmed-official`; generation remains capability-graded | intent/config possible                       | managed/user state not controlled         | generate compatible shared intent plus Doctor/manual activation  |

...
Phase-11 lift (subagents): `docs/specs/phase-11/001-subagents-schema.md`
defines the live, client-neutral `capabilities.delegation.subagents` shape
that replaces the reserved single-field block above. The runtime schema now
accepts `subagents.enabled: true` with the agent intent list defined in
`phase-11/001-subagents-schema.md`. Other reserved capability blocks remain
reserved.
...
| `memoryGuidance` | boolean | no       | gate for phase-23 memory-guidance topic on Tabnine and `AGENTS.md`  |

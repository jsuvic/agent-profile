# Spec: Codex Workflow Skills Target
...
and compiler test run. Official Codex skills docs were re-checked before
implementation. Earlier approval confirmed the `.agents/skills` path against
the official Codex skills docs and added a compiler-determinism input
cross-reference. The doctor skill-check spec
(`phase-04/006-doctor-skill-checks.md`) was amended to scan
`.agents/skills/**/SKILL.md` before this target was implemented.
Phase 10.5 later advanced only `tdd-change` to template `@2`.

## Problem

The MVP plan calls for generated workflow skills, but there is no Codex target
spec for them yet. Without a spec, the compiler cannot safely add reusable SDD,
TDD, and final-review procedures for Codex, and it risks using stale local
paths such as `.codex/skills/` instead of the current documented repository
skill path.

## Goal

Generate deterministic repository-scoped Codex workflow skills from
...
- generating scripts, references, or assets
- installing curated or third-party skills
- writing user, admin, or system skills
- generating hooks, subagents, or plugin packaging
- pre-approving tools or changing Codex permissions

## User Flow

1. A user enables `clients.codex.enabled: true`.
2. A user enables one or more workflow flags in `ai-profile.yaml`.
3. A future compile command renders project-local Codex skills under
   `.agents/skills/`.
4. Codex discovers repository skills from `.agents/skills` when started in the
   repository.
...
- current official Codex skills docs verified before implementation
- doctor skill checks from `phase-04/006-doctor-skill-checks.md`

## Outputs

- target id: `codex-workflow-skills`
...
- golden fixture outputs under
  `fixtures/minimal-valid/expected/.agents/skills/`
...
| -------------------------------------- | ---------------------------------------------- |
| `.agents/skills/sdd-change/SKILL.md`   | `targets/codex-workflow-skills/sdd-change@1`   |
| `.agents/skills/tdd-change/SKILL.md`   | `targets/codex-workflow-skills/tdd-change@2`   |
...
id is `targets/codex-workflow-skills/tdd-change@2`, while `sdd-change` and
...
`.agents/skills/sdd-change/SKILL.md` golden output must be:
...
The exact `.agents/skills/tdd-change/SKILL.md` golden output must be:
...
The exact `.agents/skills/final-review/SKILL.md` golden output must be:
...
Verified Codex skills surface for this spec revision:

Verification date: 2026-05-03.

- Codex skills are directories containing a required `SKILL.md`.
- `SKILL.md` must include `name` and `description`.
- Skills may include optional `scripts/`, `references/`, `assets/`, and
  `agents/openai.yaml`, but those are out of MVP scope.
- Codex can invoke skills explicitly with `$skill` mention or implicitly from
  the `description`.
- Repository skill locations are `.agents/skills` directories from the current
  working directory up to the repository root.
- User, admin, and system skill scopes exist, but generated MVP output is
  project-local only.
- Codex docs recommend focused skills, concise trigger descriptions, explicit
  inputs/outputs, and avoiding scripts unless deterministic tooling is needed.
...
Current official Codex docs use `.agents/skills`, not `.codex/skills`.
Therefore this target must generate `.agents/skills/**/SKILL.md`. The doctor
spec and implementation have been amended to scan `.agents/skills/**/SKILL.md`
and to ignore the legacy `.codex/skills/**/SKILL.md` path.

Mapping rules:

| Profile data           | Codex skill output                         |
| ---------------------- | ------------------------------------------ |
| `workflow.sdd: true`   | `.agents/skills/sdd-change/SKILL.md`       |
| `workflow.tdd: true`   | `.agents/skills/tdd-change/SKILL.md`       |
| `workflow.finalReview` | `.agents/skills/final-review/SKILL.md`     |
...
- Before implementing this target, re-check the current official Codex skills
  docs.
- If documented paths, required metadata, or invocation behavior differ from
  this spec, update this spec and golden fixtures before writing target code.
- Do not emit `agents/openai.yaml`, dependencies, MCP tool declarations,
  scripts, references, or assets without a later approved spec.
...
- The target must generate only project-local `.agents/skills/**/SKILL.md`
  files.
- The target must not generate `.codex/skills/**/SKILL.md` unless current
  official Codex docs change back to that path.
- The target must not pre-approve tools, install dependencies, or configure
  plugins.
- The target must not include scripts, dynamic shell commands, MCP dependencies,
...
- Do not bundle scripts or dependency declarations in MVP skills.
- Do not generate user, admin, system, or plugin skill artifacts.

## Acceptance Criteria

- `codex-workflow-skills` target contract exists.
...
- Doctor skill checks are updated for `.agents/skills/**/SKILL.md`.
...
- disabled Codex client rejects explicit `codex-workflow-skills` target with
  `disabled_target`
...
- generated output does not contain source upload, production access,
  dependency auto-install, unsafe auto-approval, `agents/openai.yaml`,
  `allowed-tools`, scripts, or MCP dependency declarations
- doctor scans `.agents/skills/**/SKILL.md` and reports skill hygiene findings
...
- `docs/specs/phase-04/006-doctor-skill-checks.md`
- `fixtures/README.md`
...
- expected skill outputs:
  `fixtures/minimal-valid/expected/.agents/skills/*/SKILL.md`
- target id assertion: `codex-workflow-skills`
...
- generated artifact shapes are concrete enough for implementation and golden
  tests
- official Codex docs were re-checked before implementation
- `.agents/skills` path is intentionally used
- doctor skill checks are updated for the current Codex path
- skills are focused, concise, and task-specific
- no scripts, dependencies, plugins, or tool pre-approvals are generated
...
Codex skill emission reads `resolveSelectedSkills(profile)`. Pack-selected
skills are written under `.agents/skills/<skill>/SKILL.md`; the Phase 12 catalog

# Spec: Tabnine Refactoring Guideline

## Status

Draft

## Problem

The verified phase-02 Tabnine output has no dedicated refactoring file.
Refactoring guidance is implicit in `00-general-agent-behavior.md`, which
mixes it with general behavior, dependency policy, and validation
preferences. Teams want a short, targeted file with explicit rules about
when to refactor, what to preserve, and what requires approval — matching
Tabnine's recommended segmentation.

## Goal

Emit a conditional, short Tabnine guideline file `70-refactoring.md` when
`workflow.refactoring` is true. The file must follow the same shape,
security, and determinism contracts as existing phase-02 outputs.

## Non-Goals

- changing the `ai-profile.yaml` schema beyond a single optional
  `workflow.refactoring` boolean
- generating Codex or Claude refactoring artifacts
- introducing a new target id
- emitting global or user-level files
- automating refactoring detection or suggestion
- duplicating the final-review block from `90-final-review.md`

## User Flow

1. The user enables refactoring guidance in `ai-profile.yaml`:

   ```yaml
   workflow:
     refactoring: true
   ```

2. `agent-profile compile --dry-run` previews the new file.
3. `--write` emits `.tabnine/guidelines/70-refactoring.md`.

## Inputs

- validated `AiProfile`
- `workflow.refactoring` flag (small additive schema field)
- derived `effectivePermissions`
- compiler determinism contract from `phase-01/003`
- golden test contract from `phase-01/005`

## Outputs

Amendment to the `phase-02/001` Output Contract adding:

| Output path                              | Template id                                  |
| ---------------------------------------- | -------------------------------------------- |
| `.tabnine/guidelines/70-refactoring.md`  | `targets/tabnine-guidelines/70-refactoring@1` |

New golden fixture under
`fixtures/refactoring-enabled/expected/.tabnine/guidelines/70-refactoring.md`.

## Generated Artifact Shape

The file begins with the standard generated-file header and covers, in
stable section order:

- refactoring principles (good reasons vs bad reasons to refactor)
- safe refactoring workflow (identify smell, check abstractions, define
  expected behavior, add tests, smallest extraction, preserve behavior,
  validation, summarize what was intentionally not changed)
- restrictions (no public API rename, no cross-module file moves, no
  schema change, no endpoint contract change, no build tooling change
  without approval)
- reference to `90-final-review.md`; do not duplicate the checklist body

The file remains under Tabnine's recommended 500-line limit.

## Contracts

- Output is conditional on `workflow.refactoring: true`.
- Absence of the flag produces no output and no warning.
- Output is byte-identical across runs given the same input.
- The file must not duplicate the final-review block from
  `90-final-review.md`.
- Output respects `effectivePermissions`.

## Security Rules

- Do not include literal secrets or environment values.
- Do not instruct Tabnine to upload source code.
- Do not grant production access during refactoring.
- Do not instruct automatic dependency installation.
- Do not instruct unsafe auto-approval.

## Acceptance Criteria

- new template id and output path appear in the phase-02 output contract
- profiles with `workflow.refactoring: true` produce the file; profiles
  without it do not
- golden output is byte-identical
- file is under 500 lines
- file references `90-final-review.md` instead of duplicating its content
- no other phase-02 outputs change

## Tests

- golden test for the new file
- absence-of-output test
- LF and trailing-newline determinism test
- secret-pattern absence test
- final-review duplication regression test

## Documentation Updates

- amend `phase-02/001` Output Contract table
- amend `phase-01/001-profile-schema-v1.md` to document the optional
  `workflow.refactoring` field
- future `docs/targets/tabnine.md`
- `fixtures/README.md`

## Final Review Checklist

- conditional render rules are deterministic
- schema change is a single optional boolean, fully backward compatible
- no duplication of shared final-review content
- file size under 500 lines
- security contract matches phase-02

...
test("legacy autonomous migration to trusted-local is explicit and previewed", async () => {
  ...
  // The legacy sandbox requirement is part of the Autonomous contract being
  // migrated away from, so it is cleared in the same transaction. Leaving it
  // would report trusted-local while the compiler kept emitting the restrictive
  // sandbox-required settings (the emission is gated on !requiresSandbox).
  assert.equal(
    await readFile(path.join(rootDir, "ai-profile.yaml"), "utf8"),
    LEGACY_AUTONOMOUS_PROFILE.replace(
      "mode: autonomous",
      "mode: trusted-local",
    ).replace("requiresSandbox: true", "requiresSandbox: false"),
  );
  assert.ok(recorded.previews[0], "migration was previewed before the write");
  await rm(rootDir, { recursive: true, force: true });
});

/** Legacy Autonomous with no explicit granular overrides, so the posture preset
 * alone drives the generated bytes and the migration's real effect is visible. */
const LEGACY_PRESET_PROFILE = PRESET_DRIVEN_PROFILE.replace(
  "  mode: guarded\n  requiresSandbox: false\n",
  "  mode: autonomous\n  requiresSandbox: true\n",
);

test("legacy migration to trusted-local actually loosens the generated Claude settings", async () => {
  // The end-to-end point of the migration. Changing safety.mode alone would
  // leave requiresSandbox: true, the compiler would fall back to the restrictive
  // baseline, and configure would report "trusted-local applied" over shared
  // settings that still force prompts and require a sandbox.
  const rootDir = await createRoot(LEGACY_PRESET_PROFILE);
  try {
    await materialize(rootDir);
    const settingsPath = path.join(rootDir, ".claude", "settings.json");
    const before = JSON.parse(await readFile(settingsPath, "utf8"));
    assert.equal(before.permissions.defaultMode, "default");
    assert.equal(before.sandbox.enabled, true);
    ...
    assert.equal(after.sandbox.enabled, false);
    // Hard denials survive the migration regardless of posture.
    assert.ok(
      after.permissions.deny.some((rule: string) => rule.includes(".env")),
      "secret denials are preserved",
    );
    assert.ok(
      report.writtenPaths.includes(".claude/settings.json"),
      "the generated settings were part of the same transaction",
    );
  } finally {
    await rm(rootDir, { recursive: true, force: true });
  }
...
test("choosing trusted-local from the legacy menu also clears the sandbox flag", async () => {
  // Same trap via the "other posture" branch: it reaches trusted-local too.
  const rootDir = await createRoot(LEGACY_PRESET_PROFILE);
  try {
    await materialize(rootDir);
    const { prompts } = scriptPrompts({
    ...
    assert.match(profile, /mode: trusted-local/u);
    assert.match(profile, /requiresSandbox: false/u);
...
test("choosing a non-trusted-local posture from the legacy menu leaves the sandbox flag alone", async () => {
  // Only trusted-local's meaning requires clearing it. Guarded is restrictive
  // anyway, so the flag stays exactly as the user's profile had it.
  const rootDir = await createRoot(LEGACY_PRESET_PROFILE);
  try {
    await materialize(rootDir);
    const { prompts } = scriptPrompts({
    ...
    assert.match(profile, /mode: guarded/u);
    assert.match(profile, /requiresSandbox: true/u);
  } finally {
    await rm(rootDir, { recursive: true, force: true });
  }
});
...
const REFUSAL_ROWS: readonly {
  ...
  {
    name: "repair-not-applicable",
    reason: "repair-not-applicable",
    exit: 1,
    script: { reconciliation: "repair", confirm: true },
    arrange: async (rootDir) => {
      await writeFile(
        path.join(rootDir, "ai-profile.yaml"),
      ...
      await materialize(rootDir);
      // Only a developer-local override diverges; configure never writes it.
      await writeClaudeLocal(rootDir, "acceptEdits");
    },
    expect: /would change nothing/u,
  },
  {
...
// ---------------------------------------------------------------------------
// Repair must be able to change something (P1b).
// ---------------------------------------------------------------------------

test("repair is refused when only a developer-local override diverges", async () => {
  // The divergence lives in .claude/settings.local.json, which this flow never
  // writes and which keeps overriding the generated file. Regenerating shared
  // artifacts cannot change the effective behavior, so reporting "applied" would
  // tell the user a mismatch was fixed while it is still there.
  const rootDir = await createRoot();
  try {
    await materialize(rootDir);
    await writeClaudeLocal(rootDir, "acceptEdits");
    ...
    assert.equal(report.outcome, "refused");
    assert.equal(report.refusal?.reason, "repair-not-applicable");
    assert.deepEqual(report.writtenPaths, []);

    // The guidance names the actual source and the manual step.
    const guidance = (recorded.refusals.at(-1)?.guidance ?? []).join("\n");
    assert.match(guidance, /settings\.local\.json/u);
    assert.match(guidance, /client itself|adopt/u);
...
test("repair applies when the generated file itself drifted", async () => {
  // A generated-project divergence IS repairable: rewriting the shared file
  // genuinely restores declared intent.
  const rootDir = await createRoot(PRESET_DRIVEN_PROFILE);
  try {
    await materialize(rootDir);
    const settingsPath = path.join(rootDir, ".claude", "settings.json");
    const canonical = await readFile(settingsPath, "utf8");
    const drifted = JSON.parse(canonical);
    drifted.permissions.defaultMode = "acceptEdits";
    await writeFile(
      settingsPath,
      `${JSON.stringify(drifted, null, 2)}\n`,
...
// ---------------------------------------------------------------------------
// An unrelated .gitignore problem must not block a posture change (P2).
// ---------------------------------------------------------------------------

test("an unreadable .gitignore does not block a posture that needs no activation", async () => {
  // balanced needs no personal activation, so .gitignore is never read, offered,
  // or written — a problem with it is none of this command's business.
  const rootDir = await createRoot(PRESET_DRIVEN_PROFILE);
  try {
    await materialize(rootDir);
    // A directory where the file belongs: any read of it throws EISDIR.
    await mkdir(path.join(rootDir, ".gitignore"), { recursive: true });
    ...
    assert.equal(report.gitignorePrerequisiteSelected, false);
    assert.ok(
      !recorded.events.includes("confirmIgnorePrerequisite"),
      "the prerequisite is never offered for a posture that does not need it",
    );
    assert.ok(!report.writtenPaths.includes(".gitignore"));
  } finally {
    await rm(rootDir, { recursive: true, force: true });

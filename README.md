...
already uses. A target file that is unowned or has drifted from the last
generated bytes refuses the whole upgrade exactly as `compile` would, writing
nothing (for example, an `ai-profile.lock` that was manually edited outside
`agent-profile` refuses the upgrade before any file is touched). A mid-write
failure rolls back every already-committed file in that batch, leaving the
repository byte-identical to before (for example, a transient write error
while renaming `ai-profile.lock` into place after `ai-profile.yaml` has
already been committed restores `ai-profile.yaml` to its prior bytes too).
